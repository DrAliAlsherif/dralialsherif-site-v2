...
(function () {
  ...
  const RART = {
    ...
    knowledge() {
      const cx = 160, cy = 80, R = 46, pts = [];
      for (let a = 0; a < 360; a += 60) {
        const rad = a * Math.PI / 180;
        pts.push([+(cx + Math.cos(rad) * R).toFixed(1), +(cy + Math.sin(rad) * R).toFixed(1)]);
      }
      let edges = "", nodes = "";
      pts.forEach((p, i) => {
        const n = pts[(i + 1) % pts.length];
        edges += `<line class="ra-edge" x1="${cx}" y1="${cy}" x2="${p[0]}" y2="${p[1]}"/>` +
                 `<line class="ra-edge" x1="${p[0]}" y1="${p[1]}" x2="${n[0]}" y2="${n[1]}"/>`;
        nodes += `<circle class="ra-node ${i % 2 ? "ra-node--b" : "ra-node--a"}" cx="${p[0]}" cy="${p[1]}" r="5"/>`;
      });
      return raWrap(edges + nodes +
        `<circle class="ra-stroke" cx="${cx}" cy="${cy}" r="13"/>` +
        `<circle class="ra-accent2" cx="${cx}" cy="${cy}" r="5"/>`);
    },
    training() {
      return raWrap(
    ...
    // An integrated library system feeding the shelf — used for Library Automation.
    ils() {
      return raWrap(
    ...
    // A paper under the lens, with citations branching off — used for Research Consulting.
    research() {
      return raWrap(
  ...
  const DATA = {
    ...
    // Each area links to its own page under /expertise/ (generated by
    // tools/gen_expertise.py). "art" picks the SVG illustration from RART.
    expertise: [
      { slug: "academic-libraries", art: "library", en: ["Academic Libraries", "Strategy, operations and services for university and research libraries."], ar: ["المكتبات الأكاديمية", "الاستراتيجية والتشغيل والخدمات لمكتبات الجامعات والبحث العلمي."] },
      { slug: "digital-repositories", art: "repository", en: ["Digital Repositories", "Planning and building institutional repositories on DSpace & Fedora."], ar: ["المستودعات الرقمية", "تخطيط وبناء المستودعات المؤسسية على DSpace وFedora."] },
      { slug: "archives-preservation", art: "archive", en: ["Archives & Preservation", "Digital archiving, security tagging and long-term preservation."], ar: ["الأرشفة والحفظ", "الأرشفة الرقمية ووسم الحماية والحفظ طويل المدى."] },
      { slug: "knowledge-management", art: "knowledge", en: ["Knowledge Management", "Capturing, organising and governing institutional knowledge."], ar: ["إدارة المعرفة", "التقاط وتنظيم وحوكمة المعرفة المؤسسية."] },
      { slug: "artificial-intelligence", art: "ai", en: ["Artificial Intelligence", "Generative-AI information services, prompt engineering & automation."], ar: ["الذكاء الاصطناعي", "خدمات المعلومات بالذكاء التوليدي وهندسة الأوامر والأتمتة."] },
      { slug: "digital-transformation", art: "technical", en: ["Digital Transformation", "Modernising information environments and workflows end-to-end."], ar: ["التحول الرقمي", "تحديث بيئات المعلومات وسير العمل بشكل متكامل."] },
      { slug: "metadata-standards", art: "metadata", en: ["Metadata Standards", "MARC21, RDA and LCSH cataloguing and metadata governance."], ar: ["معايير الميتاداتا", "الفهرسة وحوكمة الميتاداتا وفق MARC21 وRDA وLCSH."] },
      { slug: "research-support", art: "manuscript", en: ["Research Support", "Aligning collections and services with teaching and research."], ar: ["دعم البحث العلمي", "مواءمة المجموعات والخدمات مع التعليم والبحث."] },
      { slug: "training-capacity", art: "training", en: ["Training & Capacity", "Designing and delivering professional development programmes."], ar: ["التدريب وبناء القدرات", "تصميم وتقديم برامج التطوير المهني."] },
      { slug: "consulting", art: "consulting", en: ["Consulting", "Advisory and implementation for knowledge institutions."], ar: ["الاستشارات", "الاستشارات والتنفيذ لمؤسسات المعرفة."] },
    ],
    ...
    // Each service links to its own page under /services/ (generated by
    // tools/gen_services.py — keep the blurbs here in sync with it).
    // "art" picks the SVG illustration from RART; "icon" the badge glyph.
    services: [
      { slug: "ai-consulting", icon: "ai", art: "ai",
        en: ["AI Consulting", "Moving your institution from scattered AI experiments to structured adoption: prioritised use cases, a shared prompt library, and governance that protects data and copyright."],
        ar: ["استشارات الذكاء الاصطناعي", "أنقل مؤسستك من التجريب الفردي للذكاء الاصطناعي إلى تبنٍّ مؤسسي منظّم: حالات استخدام مُرتّبة بالأولوية، ومكتبة أوامر موحّدة، وحوكمة واضحة تحمي البيانات وحقوق المؤلف."] },
      { slug: "library-automation", icon: "library", art: "ils",
        en: ["Library Automation", "Choosing and deploying the right integrated library system — Koha, Symphony, Horizon — with data migration, circulation setup and staff training."],
        ar: ["أتمتة المكتبات", "اختيار نظام المكتبة المتكامل الأنسب وتنفيذه من الصفر — كوها وسيمفوني وهورايزن — مع ترحيل البيانات وضبط الإعارة والفهرسة وتدريب الفريق على التشغيل."] },
      { slug: "digital-repositories", icon: "repo", art: "repository",
        en: ["Digital Repository Development", "End-to-end DSpace and Fedora repositories: architecture, metadata schema, deposit workflows, persistent identifiers and open-access policy."],
        ar: ["تطوير المستودعات الرقمية", "بناء مستودع رقمي مؤسسي على DSpace أو Fedora من دراسة الاحتياج إلى الإطلاق: البنية والميتاداتا وسير الإيداع والمعرّفات الدائمة وسياسات الوصول الحر."] },
      { slug: "archive-consulting", icon: "archive", art: "archive",
        en: ["Archive Consulting", "Organising the institutional archive: classification and retention schedules, quality-controlled digitisation, security tagging and long-term preservation."],
        ar: ["استشارات الأرشيف", "تنظيم أرشيف المؤسسة رقميًا: خطة تصنيف وجداول احتفاظ، ورقمنة بمواصفات جودة، ووسم حماية وضبط وصول، وحفظ رقمي طويل الأمد يضمن سلامة الوثيقة عبر الزمن."] },
      { slug: "metadata-consulting", icon: "metadata", art: "metadata",
        en: ["Metadata Consulting", "Raising cataloguing quality with MARC 21, RDA and LCSH — a unified policy, authority control, batch editing and metrics that make content findable."],
        ar: ["استشارات الميتاداتا", "رفع جودة الفهرسة والميتاداتا وفق MARC 21 وRDA وLCSH: سياسة موحّدة، وضبط استناد، وتحرير بالدُّفعات، ومؤشرات جودة تجعل المحتوى قابلًا للاكتشاف فعلًا."] },
      { slug: "training-programs", icon: "training", art: "training",
        en: ["Training Programs", "Professional development built around your team's actual gaps — competency analysis, hands-on Arabic workshops, train-the-trainer and impact measured on performance."],
        ar: ["البرامج التدريبية", "برامج تطوير مهني مُفصَّلة على احتياج فريقك: تحليل كفايات، وحقائب تدريبية عملية بالعربية، وتدريب مدرّبين، وقياس أثر على الأداء لا على الحضور."] },
      { slug: "research-consulting", icon: "research", art: "research",
        en: ["Research Consulting", "Support across the research lifecycle — literature reviews, publishing and impact metrics, FAIR data-management plans, and evaluation and foresight studies."],
        ar: ["الاستشارات البحثية", "دعم دورة البحث كاملة: مراجعة الأدبيات وإدارة المراجع والنشر ومقاييس الأثر، وخطط إدارة بيانات البحث وفق مبادئ FAIR، ودراسات التقييم والاستشراف."] },
      { slug: "digital-transformation", icon: "transform", art: "strategy",
        en: ["Digital Transformation", "An executable roadmap — maturity assessment, budgeted priorities, process redesign and automation, and change management that gets the team on board."],
        ar: ["التحول الرقمي", "خارطة طريق تحوّل رقمي قابلة للتنفيذ: تقييم نضج، وأولويات مرتبطة بموازنة وجدول زمني، وإعادة تصميم عمليات وأتمتة، وإدارة تغيير تضمن تبنّي الفريق."] },
    ],
  ...
  const I18N = {
    en: {
      ...
      "services.sub": "Advisory, implementation and capacity-building for institutions modernising their knowledge infrastructure. Open any service for what it covers, what you receive and how the work runs.",
      "services.all": "Browse every service in detail",
      ...
      "expertise.all": "Browse every area & its services",
    ...
    ar: {
      ...
      "services.sub": "الاستشارات والتنفيذ وبناء القدرات للمؤسسات التي تحدّث بنيتها المعرفية. افتح أي خدمة للاطلاع على ما تشمله والمخرجات التي تتسلّمها ومنهجية العمل.",
      "services.all": "تصفّح كل خدمة بالتفصيل",
      ...
      "expertise.all": "تصفّح كل مجال والخدمات المقدَّمة فيه",
  ...
  // The Arabic mirror lives one level down at /ar/, so sub-page links rendered
  // from here need to climb back to the site root.
  const ROOT = /(^|\/)ar\/(index\.html)?$/.test(location.pathname) ? "../" : "";
    ...
    $("#expertiseGrid").innerHTML = DATA.expertise.map((x, i) => {
      ...
      return `<article class="xcard reveal" style="transition-delay:${(i % 4) * 40}ms">
        ...
        <div class="xcard-body">
          ...
          <p>${de}</p>
          <a class="xcard-more" href="${href}">${more}
    ...
    $("#servicesGrid").innerHTML = DATA.services.map((x, i) => {
      ...
      return `<article class="scard reveal" style="transition-delay:${(i % 4) * 40}ms">
        ...
        <div class="scard-body">
          ...
          <p>${de}</p>
          <a class="scard-more" href="${href}">${more}
            <svg viewBox="0 0 24 24" aria-hidden="true" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 6l6 6-6 6"/></svg>
          </a>
        </div>
      </article>`;
