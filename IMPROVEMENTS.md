...
| 18 | Expertise: per-area service pages + concept art | ✅ | `#expertise` cards now show a bespoke SVG illustration (not an icon) and link to `expertise/<slug>.html` — a blurb, the services offered, the working approach and who it's for. Generated by `tools/gen_expertise.py`; `expertise/index.html` has `ItemList` JSON-LD, each page has `Service` JSON-LD |
| 19 | Book samples + page-flip reader | ✅ | Each book in `#books` has a **"تصفّح مقدمة الكتاب"** button opening `books/viewer.html` — a real page-turn flipbook (StPageFlip) that rasterises the sample PDF with pdf.js. Lazy per-page rendering, 2-page spread on desktop / single on mobile, zoom + fullscreen + open-PDF, per-page timeout→skip, and an `<iframe>` fallback if the libraries are blocked. Libraries and PDFs are vendored under `assets/vendor/` and `assets/books/`. Book `sample:` paths live in `DATA.books` |
| 20 | Services: per-service pages + concept art | ✅ | Each `#services` card now carries its own SVG illustration, a real blurb and a link to `services/<slug>.html` — an expanded introduction, what the engagement covers, the deliverables handed over, the working approach and who it is for, cross-linked to the matching expertise area. Generated by `tools/gen_services.py`; `services/index.html` has `ItemList` JSON-LD, each page has `Service` JSON-LD. Two new illustrations (`ils`, `research`) were added to `RART` in `main.js` |
| 21 | English versions of every sub-page | ✅ | The home page was bilingual but every sub-page was Arabic only, so an English visitor clicking through landed in Arabic. Each generator now emits both languages: `services/en/` (8), `expertise/en/` (10), `workshops/en/` (14) and `articles/en/`, each paired with its Arabic twin via `hreflang` + a header switch link, and `main.js` routes cards by active language. `books/viewer.html` takes `?lang=`. Illustrations moved to the shared `tools/_art.py`; the stale duplicate `workshops/assets/workshop.css` was dropped in favour of `assets/css/subpage.css`. `media-kit.html` stays a single bilingual page by design |
| 22 | Real photos on the workshop cards | ◑ | The card art was picked by keyword, so 7 drawings covered 14 workshops and four shared one neural-net. Cards now layer a photo from `assets/img/workshops/<slug>.jpg` over that SVG, gated on `WORKSHOP_PHOTOS` so a workshop without one costs no failed request. `tools/prepare_workshop_photos.py` crops sources to the 16:7.4 card ratio, compresses under 180KB and syncs the list into `main.js`. **6 of 14 are live**; the other 8 are listed in `HOLD` with the reason — burnt-in English wording on a bilingual page, prompt scaffolding drawn as UI chrome, white grounds in a dark grid, or wrong-domain content — and keep their SVG until reshot. See `tools/workshop-image-prompts.md` |

## Build commands

```bash
python tools/gen_workshops.py     # regenerate the 14 workshop pages (ar + en) + indexes
python tools/gen_expertise.py     # regenerate the 10 expertise pages (ar + en) + indexes
python tools/gen_services.py      # regenerate the 8 service pages (ar + en) + indexes
...
Run the `gen_*` scripts then `build.py` after editing `index.html`, workshop or
expertise content, and commit the generated files.
