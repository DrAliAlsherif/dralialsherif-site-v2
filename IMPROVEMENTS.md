...
| 22 | Real photos on the workshop cards | ✅ | The card art was picked by keyword, so 7 drawings covered 14 workshops and four shared one neural-net. Every workshop now has its own photo in `assets/img/workshops/<slug>.jpg`, layered over a bespoke SVG that remains as the load-failure fallback. `tools/prepare_workshop_photos.py` crops sources to the 16:7.4 card ratio, compresses under 180KB and syncs `WORKSHOP_PHOTOS` in `main.js`. The first batch had 8 rejects (burnt-in English wording on a bilingual page, prompt scaffolding drawn as UI chrome, white grounds, wrong-domain content); rewriting those prompts as scenes rather than diagrams fixed all 8. See `tools/workshop-image-prompts.md` |
